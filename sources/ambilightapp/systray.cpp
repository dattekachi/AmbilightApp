...
#else
QSettings SysTray::s_settings("AmbilightApp", "AmbilightApp");
#endif
...
bool SysTray::getCurrentAutorunState()
{
	QString plistPath = QDir::homePath() + "/Library/LaunchAgents/com.ambilightapp.autorun.plist";
	bool exists = QFile::exists(plistPath);
	if (exists) {
		_autorunAction->setText(tr("&Tắt tự khởi động"));
		return true;
	}
	...
	if (currentState) {
		...
		if (file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
			QString plist =
				...
				"<dict>\n"
				"\t<key>Label</key>\n"
				"\t<string>com.ambilightapp.autorun</string>\n"
...
void SysTray::runMusicLed()
{
	...
	QString musicLedPath = QCoreApplication::applicationDirPath() + "/../Resources/MusicLedStudio.app";
	process.start("open", QStringList() << musicLedPath);
